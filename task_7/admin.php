    ...
    <table id="data" class="min-w-full bg-white border border-gray-200">
        ...
        while ($row = $dbFD->fetch(PDO::FETCH_ASSOC)) {
            echo '<tr class="border-b border-gray-200 hover:bg-gray-100" data-id=' . checkInput($row['id']) . '>
                            ...
                            <td class="py-3 px-4">' . ((checkInput($row['gender']) == "male") ? "Мужской" : "Женский") . '</td>
                            <td class="py-3 px-4 whitespace-normal">' . checkInput($row['biography']) . '</td>
                            <td class="py-3 px-4">';
            ...
            while ($row1 = $dbl->fetch(PDO::FETCH_ASSOC)) {
                echo checkInput($row1['name']) . '<br>';
            }
            echo '</td>
                            <td class="py-3 px-4"><a href="./index.php?uid=' . checkInput($row['user_id']) . '" target="_blank" class="text-blue-500 hover:underline">Редактировать</a></td>
    ...
    <table class="analize mt-8 min-w-full bg-white border border-gray-200" id="analize">
        ...
        while ($row = $qu->fetch(PDO::FETCH_ASSOC)) {
            echo '<tr class="border-b border-gray-200 hover:bg-gray-100">
                            <td class="py-3 px-4">' . checkInput($row['name']) . '</td>
                            <td class="py-3 px-4">' . checkInput($row['count']) . '</td>
                          </tr>';
        }
        ?>
        </tbody>
